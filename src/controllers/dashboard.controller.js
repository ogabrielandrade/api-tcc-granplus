...
const getDashboardResume = async (req, res) => {
  try {
    const [totalProdutosAtivos] = await pool.execute(`
                SELECT COUNT(*) AS total_produtos
                FROM produto
                WHERE pdt_ativo = 1
        `);

    const [produtosEstoqueMinimo] = await pool.execute(`
    ...
    res.json({
      total_produtos: totalProdutosAtivos[0].total_produtos,
      produtos_abaixo_minimo: produtosEstoqueMinimo[0].abaixo_minimo,
